...
void Pipeline::CreateParticlePipline(Blob& blob, BlendMode blend, ID3D12Device* device, std::array<PipelineSet, 6>& pip)
{
	HRESULT result;
	...
	pipelineDesc.PS.pShaderBytecode = blob.ps->GetBufferPointer();
	pipelineDesc.PS.BytecodeLength = blob.ps->GetBufferSize();
	pipelineDesc.GS.pShaderBytecode = blob.gs->GetBufferPointer();
	pipelineDesc.GS.BytecodeLength = blob.gs->GetBufferSize();
	// サンプルマスクの設定
	pipelineDesc.SampleMask = D3D12_DEFAULT_SAMPLE_MASK; // 標準設定
	// ラスタライザの設定
	pipelineDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE; // カリングしない
...
void Pipeline::CreateMultiPhongModelPipline(Blob& blob, BlendMode blend, ID3D12Device* device, std::array<PipelineSet, 6>& pip)
{
	HRESULT result;
	...
	// ブレンドステート
	D3D12_RENDER_TARGET_BLEND_DESC blenddesc{};// RBGA全てのチャンネルを描画
...
void Pipeline::CreatePostEffectPipline(Blob& blob, PipelineSet& pip)
{
	HRESULT result;
